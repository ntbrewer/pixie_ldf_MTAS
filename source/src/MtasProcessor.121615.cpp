...
void MtasProcessor::DeclarePlots(void) const
{
	...
	{    	
		...
		//SVP 12-26-2015 - Added new histograms to work without the logic signals, only the beta gating
		DeclareHistogram1D(MTAS_POSITION_ENERGY+600+i, EnergyBins, ("Total Mtas, Beta-gated"+titlePart).c_str());
		DeclareHistogram1D(MTAS_POSITION_ENERGY+610+i, EnergyBins, ("Total Central, Beta-gated"+titlePart).c_str());
		DeclareHistogram1D(MTAS_POSITION_ENERGY+620+i, EnergyBins, ("Total Inner, Beta-gated"+titlePart).c_str());
		DeclareHistogram1D(MTAS_POSITION_ENERGY+630+i, EnergyBins, ("Total Middle, Beta-gated"+titlePart).c_str());
		DeclareHistogram1D(MTAS_POSITION_ENERGY+640+i, EnergyBins, ("Total Outer, Beta-gated"+titlePart).c_str());
	}
...
//"Regular" measurement 
    ...
    //! SVP 12-16-2015 - Added to remove the issue of no beta gating for the MTAS spectra.
    for(int i=0; i<5; i++)
      if(isBetaSignal)
	plot(MTAS_POSITION_ENERGY+600+i*10, totalMtasEnergy.at(i));
    ...
    if(isMeasureOn && !isBkgOn && !isLightPulserOn && !isTapeMoveOn)
      {
		...
		for(int i=0; i<5; i++)
		{
			...
			if(isBetaSignal) {
				...
				if (i==1) {
        			plot(MTAS_POSITION_ENERGY+306, (actualTime - firstTime));//NTB 12/15/15
				}
			}    
		}
